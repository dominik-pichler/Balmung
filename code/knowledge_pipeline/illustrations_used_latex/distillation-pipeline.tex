...
%% =====================================================
%% Push into Knowledge Graph
%% =====================================================
% Small graph icon: 5 nodes with edges (positioned well below the distillate)
\node[circle, fill=accent, minimum size=3.5mm, inner sep=0] (kg1) at (-1.4, -11.0) {};
\node[circle, fill=accent, minimum size=3.5mm, inner sep=0] (kg2) at ( 0.2, -10.7) {};
\node[circle, fill=accent, minimum size=3.5mm, inner sep=0] (kg3) at ( 1.6, -11.1) {};
\node[circle, fill=accent, minimum size=3.5mm, inner sep=0] (kg4) at (-0.6, -11.8) {};
\node[circle, fill=accent, minimum size=3.5mm, inner sep=0] (kg5) at ( 1.0, -11.8) {};

\foreach \a/\b in {kg1/kg2, kg2/kg3, kg1/kg4, kg2/kg4, kg2/kg5, kg3/kg5, kg4/kg5}
    \draw[accent, line width=0.7pt] (\a) -- (\b);

% Label sits centred above the graph icon (clear of the distillate)
\node[font=\small\bfseries, text=accent] (KGLAB) at (0, -9.7) {KNOWLEDGE GRAPH};

% Container around the graph icon + its label
\begin{scope}[on background layer]
    \node[draw=accent!70, fill=accent!5, rounded corners=5pt,
          line width=0.9pt,
          inner xsep=16mm, inner ysep=5mm,
          fit=(KGLAB) (kg1) (kg2) (kg3) (kg4) (kg5)] (KGBOX) {};
\end{scope}

% Bold flow arrow: Distillate → Knowledge Graph
\draw[keyflow, line width=1.6pt] (DIST.south) -- (KGBOX.north);
