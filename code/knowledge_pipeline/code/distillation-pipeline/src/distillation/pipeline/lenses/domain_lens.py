...
class DomainLens(Lens[DomainResponse, ExtractedEntity]):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "robustness, safety). For each capability set 'addresses' to the "
            "exact name of the Problem it addresses, when the text states it "
            "(use the same short name you gave that problem).\n"
            ...
            "- limitations: constraints on a technology or result. For each "
            "limitation set 'concerns' to the exact name of the technology, "
            "problem, or other entity it constrains (use the same short name "
            "used elsewhere).\n\n"
            "Use consistent, short names so entities referenced across fields "
            "match. Omit fields you cannot determine. Provide a 'confidence' in "
            "[0,1] reflecting extraction certainty (never 1.0 unless verbatim)."
        )
