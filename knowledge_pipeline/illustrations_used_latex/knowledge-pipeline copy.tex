...
%% Flow is BOTTOM → TOP: distillates enter at the bottom (Aggregation),
%% are compressed through the coil (Thinking), and emerge as Action at top.
%% ============================================================
\def\BX{3.35} % bracket x-position

% Action bracket — top funnel (where the synthesised action emerges)
\draw[outline] (\BX,6.95) -- ++(0.15,0) -- ($(\BX+0.15, 4.25)$) -- ++(-0.15,0);
\node[sectionlabel] at (\BX+1.10, 5.6) {Action};

% Thinking bracket — coil/spring waist
\draw[outline] (\BX,4.10) -- ++(0.15,0) -- ($(\BX+0.15, 3.05)$) -- ++(-0.15,0);
\node[sectionlabel] at (\BX+1.30, 3.6) {Thinking};

% Aggregation bracket — bottom funnel (agents pulling distillates together)
\draw[outline] (\BX,2.90) -- ++(0.15,0) -- ($(\BX+0.15, 0.5)$) -- ++(-0.15,0);
\node[sectionlabel] at (\BX+1.55, 1.7) {Aggregation};
