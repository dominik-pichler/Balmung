...
from distillation.domain.distillate import AssumptionMention, LensOutput
from distillation.domain.document import Chunk
from distillation.pipeline.lenses.assumption import (
    AssumptionLens,
    AssumptionResponse,
    _AssumptionItem,
)
...
async def test_lens_returns_empty_for_no_chunks(fake_llm: FakeLLMClient):
    lens = AssumptionLens(fake_llm)
    out = await lens.apply([])
    assert out == LensOutput(lens_name="assumption", items=[])
...
async def test_lens_returns_items_with_provenance(
    ...
    fake_llm.set_response(
        ...
        AssumptionResponse(
            assumptions=[
                _AssumptionItem(name="Data is IID", statement="The data is IID."),
                _AssumptionItem(name="Clean Labels", statement="Labels are noise-free."),
            ]
        ),
    )
    ...
    assert out.lens_name == "assumption"
    assert len(out.items) == 2
    assert all(isinstance(i, AssumptionMention) for i in out.items)
